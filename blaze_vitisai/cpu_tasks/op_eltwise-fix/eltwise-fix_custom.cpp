...
struct MyOpImp : public vart::experimental::OpImpBase {
  ...
  int calculate(vart::simple_tensor_buffer_t<int8_t> output,
    ...
    switch (type_) {
      ...
      case MUL:
        ...
        break;
      case DIV:
        eltwise_div(input_ptr_vec, output.data, element_num);
        break;
      default:
        exit(1);
    }
 ...
 private:
  ...
  void eltwise_div(const std::vector<int8_t*>& input, int8_t* output,
                   int element_num) {
    auto empty_map = input_diff_shape_map_.empty();
    
    auto input_num = static_cast<int>(input.size());

    //std::cout << "[eltwise_div] element_num=" << element_num << std::endl;
    //std::cout << "[eltwise_div] input_num=" << input_num << std::endl;
    //std::cout << "[eltwise_div] empty_map=" << empty_map << std::endl;
    for (auto idx = 0; idx < element_num; ++idx) {
      float data;
      int index;
      index = 0;
      float data1 = input[index][idx] * input_scale_vec_[index];
      //printf("[eltwise_div] input[%d][%d] * input_scale_vec[%d] => %f * %f => %f\n", 0, idx, 0, (double)input[index][idx], input_scale_vec_[index], data1);
      index = 1;
      float data2 = input[1][idx] * input_scale_vec_[1];
      //printf("[eltwise_div] input[%d][%d] * input_scale_vec[%d] => %f * %f => %f\n", 0, idx, 0, (double)input[index][idx], input_scale_vec_[index], data2);
      data = data1 / data2;
      //printf("[eltwise_div] %f / %f => %f\n", data1, data2, data);
      output[idx] = vitis::ai::cpu_task::util::fix(nonlinear(data * output_scale_));
      //printf("[eltwise_div] data * output_scale_vec[%d] => %f * %f => %d\n", 0, data, output_scale_, output[idx]);
    }
    return;
  }
